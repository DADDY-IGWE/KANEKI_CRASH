...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      setTimeout(async () => {
         ...
         console.log(chalk.black(chalk.bgGreen(`𝚢𝚘𝚞𝚛 𝚌𝚘𝚍𝚎 𝚖𝚊𝚜𝚝𝚎𝚛 : `)), chalk.black(chalk.white(code)))
      }, 3000)
   }
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
	        ...
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: Davidjunior`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: DADDY-IGWE`))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @davidjunior `))
